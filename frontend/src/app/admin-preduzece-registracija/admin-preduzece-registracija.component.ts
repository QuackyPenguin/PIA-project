...
@Component({
  selector: 'app-admin-preduzece-registracija',
  templateUrl: './admin-preduzece-registracija.component.html',
  styleUrls: ['./admin-preduzece-registracija.component.css']
})
export class AdminPreduzeceRegistracijaComponent implements OnInit {
  ...
  registracija() {
    if (this.validacija())
      this.preduzeceService.registracija(this.odgovorno_lice, this.kor_ime, this.lozinka, this.telefon, this.imejl,
        ...
        this.ulica, this.broj, this.pib, this.maticni, 2, this.slika).subscribe((res) => {
          switch (res['flag']) {
            case 0: this.porukaRegistracija = "Preduzece je registrovano"; break;
